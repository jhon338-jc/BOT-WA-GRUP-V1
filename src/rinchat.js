...
export async function createRinChat(teks, imgUrl = null, timeStr = null) {
    try {
        ...
        const finalTime = timeStr || new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false }).replace(':', '.')
        ...
        for (const f of RIN_FONTS) {
            ...
            if (!existsSync(dest)) {
                console.log(`[⬇️] Mengunduh font Inter-Regular...`);
                await writeFile(dest, await rinDownload(f.url));
                console.log('[✅] Font Inter-Regular berhasil diunduh');
            }
            GlobalFonts.registerFromPath(dest, 'InterRegular')
        }

        if (!existsSync(RIN_BG_LOCAL)) {
            console.log('[⬇️] Mengunduh background Rinchats...');
            await writeFile(RIN_BG_LOCAL, await rinDownload(RIN_BG_URL));
            console.log('[✅] Background Rinchats berhasil diunduh');
        }
        ...
        ctx.fillText(finalTime, PERMANENT_TIME_X, PERMANENT_TIME_Y);
        ...
        const timeWidth = ctx.measureText(finalTime).width;

        let finalY, finalBubbleHeight, bubbleW;

        if (!imgUrl) {
            ctx.font = `${chatFontSize}px InterRegular`;
            const chatLines = wrapText(ctx, teks, maxWidthLimit, chatFontSize);
            ...
            ctx.textAlign = "right";
            ctx.textBaseline = "top";
            ctx.fillText(finalTime, fixedX + bubbleW - 22, finalY + finalBubbleHeight - 38);
            ...
            if (teks) {
                ctx.font = `${chatFontSize}px InterRegular`;
                captionLines = wrapText(ctx, teks, bubbleW - paddingX * 2, chatFontSize);
            }
            ...
            ctx.textAlign = "right";
            ctx.textBaseline = "middle";
            ctx.fillText(finalTime, fixedX + bubbleW - 22, finalY + finalBubbleHeight - timeRowH);
        }
        ...
        console.error('❌ Error createRinChat:', e.message || e)
        throw new Error(`Gagal buat Rinchats: ${e.message}`);
    }
}